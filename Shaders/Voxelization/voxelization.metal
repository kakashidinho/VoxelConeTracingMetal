...
vertex VS_out VS(const device VS_in *vertices VERTEX_BUFFER_BINDING,
                 ...
                 constant VoxelProjectionDir& projDir VOXEL_PROJ_BINDING)
{
    ...
    if(triNormal.z > triNormal.x && triNormal.z > triNormal.y){
        if (projDir.direction == 2)
            out.gl_Position = float4(out.worldPosition.x, out.worldPosition.y, 0, 1);
    } else if (triNormal.x > triNormal.y && triNormal.x > triNormal.z){
        if (projDir.direction == 0)
            out.gl_Position = float4(out.worldPosition.y, out.worldPosition.z, 0, 1);
    } else {
        if (projDir.direction == 1)
            out.gl_Position = float4(out.worldPosition.x, out.worldPosition.z, 0, 1);
    }
